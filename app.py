...
app.config["JWT_SECRET_KEY"] = os.getenv(
    "JWT_SECRET_KEY"
)  # chiave per la generazione dei token
...
def configure_logger():
    ...
    if logger is None:
        ...
        file_handler = TimedRotatingFileHandler(
            log_file, when="M", interval=1, backupCount=3
        )
...
def cerca():
    ...
    if not ricerca:
        return render_template(
            "error.html", errore=1, messaggio="Termine di ricerca non valido."
        )

    conn, cursor = db.connection_establish()
    if not conn:
        return render_template(
            "error.html", errore=500, messaggio="Errore di connessione al database."
        )
    ...
    try:
        ...
        """
        logger.debug(
            f"IP: {get_ip()} Esecuzione query: {query} con parametri : {(pattern)*6}"
        )
        cursor.execute(
            query, (pattern,) * 6
        )  # modo più compatto per ripetere il pattern
        ...
        if not risultati:
            return render_template(
                "error.html",
                errore=404,
                messaggio="Nessun risultato trovato per la ricerca.",
            )
        ...
        return render_template(
            "risultati.html", risultati=risultati_list, ricerca=ricerca
        )

    except Exception as e:
        print(f"Errore del database: {e}")
        return render_template(
            "error.html", errore=500, messaggio="Si è verificato un errore interno."
        )
...
def login():
    ...
    try:
        logger.debug(
            f"Esecuzione query login dall'IP : {get_ip()} con parametri: {email}"
        )
        ...
        access_token = create_access_token(
            identity=email,
            additional_claims={"iat": iat.timestamp(), "exp": exp.timestamp()},
        )
...
def inserisci_libro():
    if request.method == "POST":
        ...
        try:
            ...
            print(f"Errore del database: {e}")
            return (
                jsonify({"success": False, "msg": "Errore durante l'inserimento"}),
                500,
            )
...
def gestisci_prestiti():
    ...
    if not conn:
        return (
            jsonify({"success": False, "msg": "Errore di connessione al database"}),
            500,
        )
    ...
    if request.method == "POST":
        ...
        if prevista < inizio:
            return (
                jsonify(
                    {
                        "success": False,
                        "msg": "La data di restituzione/prevista deve essere successiva alla data di prestito",
                    }
                ),
                400,
            )
        ...
        try:
            ...
            if disponibile != 1:
                return (
                    jsonify(
                        {
                            "success": False,
                            "msg": "Libro non disponibile perché già in prestito",
                        }
                    ),
                    400,
                )
            ...
            cursor.execute(
                "UPDATE catalogo SET disponibile = 0 WHERE id = %s", (id_libro,)
            )
            conn.commit()
            return (
                jsonify({"success": True, "msg": "Prestito aggiunto con successo"}),
                201,
            )
            ...
            return (
                jsonify(
                    {
                        "success": False,
                        "msg": "Errore durante l'inserimento del prestito",
                    }
                ),
                500,
            )
...
"""
        ...
        return render_template(
            "gestisci_prestiti.html", utenti=utenti, prestiti=prestiti, today=today
        )

    except Exception as e:
        print(f"Errore del database: {e}")
        return (
            jsonify({"success": False, "msg": "Errore durante il recupero dei dati"}),
            500,
        )
        ...
        if (
            conn
        ):  # Aggiunta per evitare errori se la connessione non è mai stata stabilita
            conn.close()
    ...
    try:
        ...
        if data_restituzione and data_restituzione < data_prestito:
            return (
                jsonify(
                    {
                        "msg": "La data di restituzione deve essere successiva alla data di prestito"
                    }
                ),
                400,
            )
...
def elimina_prestito(id):
    ...
    try:
        ...
        if max[0] == id:
            ...
            conn.commit()
            cursor.execute(
                "UPDATE catalogo SET disponibile=1 WHERE id =%s", (idlibro[0],)
            )
            conn.commit()
        else:
            cursor.execute("DELETE FROM prestiti WHERE id = %s", (id,))
...
def download_prestiti_excel():
    ...
    header_format = workbook.add_format(
        {"bold": True, "bg_color": "#A9A9A9", "border": 1}
    )
...
def nuovo_utente():
    if request.method == "POST":
        ...
        try:
            ...
            cursor.execute(
                "SELECT id FROM utenti WHERE codice_fiscale = %s", (codice_fiscale,)
            )
            if cursor.fetchone():
                return (
                    jsonify(
                        {
                            "success": False,
                            "msg": "Codice fiscale già presente nel database",
                        }
                    ),
                    400,
                )
            ...
            """
            cursor.execute(
                query, (email, codice_fiscale, nome_cognome, numero_telefono)
            )
            conn.commit()
    ...
    try:
        ...
        if not conn:
            return jsonify({"msg": "Errore di connessione al database"}), 500
        cursor.execute(
            "SELECT id, codice_fiscale, nome_cognome, email, telefono FROM utenti"
        )
        utenti = cursor.fetchall()
...
def modifica_utente(id):
    ...
    try:
        ...
        cursor.execute(
            "SELECT id FROM utenti WHERE email = %s AND id != %s", (email, id)
        )
        if cursor.fetchone():
            return jsonify({"success": False, "msg": "Email già in uso"}), 400
        ...
        cursor.execute(
            "SELECT id FROM utenti WHERE codice_fiscale = %s AND id != %s",
            (codice_fiscale, id),
        )
        if cursor.fetchone():
            return (
        ...
        """
        cursor.execute(
            query, (email, codice_fiscale, nome_cognome, numero_telefono, id)
        )
        conn.commit()
        ...
        if "foreign key constraint fails" in str(e):
            return (
                jsonify(
                    {
                        "success": False,
                        "msg": "Impossibile eliminare l'utente perche' ha prestiti registrati",
                    }
                ),
                400,
            )
            ...
            print(
                f"Errore inatteso durante l'eliminazione utente: {e}"

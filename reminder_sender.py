...
def controllo_db():
    try:
        ...
        cursor.execute(
            """SELECT id_libro, id_utente FROM prestiti WHERE promemoria_inviato=0 AND DATEDIFF(data_restituzione_prevista,CURRENT_DATE())=5 AND data_restituzione is null"""
        )
        ...
        for tupla in query_avaiable:
            ...
            query_email.append(
                cursor.fetchone()
            )  # Appendo nella lista il fetch singolo di ogni iterazione
        ...
        for tupla in query_avaiable:
            ...
            query_corresponding.append(
                cursor.fetchone()
            )  # Appendo nella lista il fetch di ogni iterazione
        # print(f"corresponding: {query_corresponding}")
        db.connection_close(connection, cursor)

        final_dict = {}  # Inizializzo il dict
        for (id_libro, id_utente), email, (isbn, nome_libro) in zip(
            query_avaiable, query_email, query_corresponding
        ):  # Zip serve a creare un unico iterabli di iterable
...
def controllo_db_scadenza():
    try:
        ...
        if not connection or not cursor:
            print("Connesione al database non riuscita")
        cursor.execute(
            """SELECT id_libro, id_utente FROM prestiti WHERE DATEDIFF(data_restituzione_prevista,CURRENT_DATE())=0 AND data_restituzione is null"""
        )
        query_avaiable_expiring = (
            cursor.fetchall()
        )  # fetchall ritorna una lista di tuple
        # print(f"avaiable: {query_avaiable}")
        query_email_expiring = []  # Inizializzo la lista
        for tupla in query_avaiable_expiring:
            cursor.execute("""SELECT email from utenti WHERE id=%s""", (tupla[1],))
            query_email_expiring.append(
                cursor.fetchone()
            )  # Appendo nella lista il fetch singolo di ogni iterazione
        # print(f"email: {query_email}")
        ...
        for tupla in query_avaiable_expiring:
            ...
            query_corresponding_expiring.append(
                cursor.fetchone()
            )  # Appendo nella lista il fetch di ogni iterazione
        # print(f"corresponding: {query_corresponding}")
        db.connection_close(connection, cursor)
        ...
        for (id_libro, id_utente), email, (isbn, nome_libro) in zip(
            query_avaiable_expiring, query_email_expiring, query_corresponding_expiring
        ):  # Zip serve a creare un unico iterabli di iterable
            key = (id_libro, isbn, nome_libro)
            value = (id_utente, email[0])
...
def controllo_db_scaduti():
    try:
        ...
        for tupla in query_avaiable_expired:
            ...
            query_corresponding_expired.append(
                cursor.fetchone()
            )  # Appendo nella lista il fetch di ogni iterazione
        # print(f"corresponding: {query_corresponding}")
        db.connection_close(connection, cursor)
        ...
        for (id_libro, id_utente), email, (isbn, nome_libro) in zip(
            query_avaiable_expired, query_email_expired, query_corresponding_expired
        ):  # Zip serve a creare un unico iterabli di iterable
            key = (id_libro, isbn, nome_libro)
            value = (id_utente, email[0])
...
def invia_reminder(final_dict, chiave):
    ...
    try:
        ...
        with smtplib.SMTP_SSL("out.virgilio.it", 465) as server:
            ...
            connection, cursor = db.connection_establish()
            cursor.execute(
                """UPDATE prestiti SET promemoria_inviato=1 WHERE id_libro=%s""",
                (chiave[0],),
            )
            connection.commit()
            db.connection_close(connection, cursor)
